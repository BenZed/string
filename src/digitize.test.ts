...
describe('digitize', () => {
    ...
    describe('decimal options', () => {
        describe('decimalPlaces', () => {
            ...
            it('must be an integer', () => {
                expect(() => digitize(PI, { decimalPlaces: 1.5 })).toThrow(
        ...
        describe('trailingZeros', () => {
            it('fills trailing zeros to decimal places', () => {
                expect(
                    digitize(1.2, { decimalPlaces: 5, trailingZeros: true })
                ).toEqual('1.20000')
            })
        })
    })
    ...
    describe('whole options', () => {
        describe('wholePlaces', () => {
            it('adds leading zeros to output', () => {
                expect(digitize(78, { wholePlaces: 5 })).toEqual('00078')
            })

            it('0 has no effect', () => {
                expect(digitize(78, { wholePlaces: 0 })).toEqual('78')
            })

            it('must be an integer', () => {
                expect(() => digitize(PI, { wholePlaces: 1.5 })).toThrow(
                    'wholePlaces must be an integer'
                )
            })
        })
        ...
        describe('truncateWhole', () => {
            it('truncates numbers that have more digits than wholePlaces', () => {
                expect(
                    digitize(12345, { wholePlaces: 4, truncateWhole: true })
                ).toEqual('2345')
            })

            it('combined with wholePlace 0, removes everything before the decimal', () => {
                expect(
                    digitize(12345.67, { wholePlaces: 0, truncateWhole: true })
                ).toEqual('.67')
            })
        })
    })
    ...
    describe('Both option types work in unison', () => {
        for (const { options, input, output } of [
            ...
            it(`${input} @${JSON.stringify(options).replace(
                /"/g,
                ''
            )} -> ${output}`, () => {
