 ...
 */
type ToDashCase<S extends string | string[]> = string extends S
    ? string
    : S extends string
...
function fromCamelCase(input: string, dash = '-'): string {
    let output = ''
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (requiresDash) output += dash

        if (isCaseAble || isDigit) output += charLo
 ...
 */
function toDashCase<S extends string>(input: S): ToDashCase<S> {
