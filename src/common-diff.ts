...
type CommonDiff = [common: string, ...diffs: string[]]
...
function createCommon(
    ...
    while (input.length > 0) {
        ...
        common = fromEnd ? char + common : common + char
    }
...
function* createDiffs(
    ...
    for (const string of input) yield string.slice(start, end)
}
...
function commonDiff(
    input: string[],
    options?: Partial<CommonDiffOptions>
): CommonDiff
...
function commonDiff(
    ...
    return [common, ...createDiffs(input, common.length + offset, fromEnd)]
}
...
export { commonDiff, CommonDiff, CommonDiffOptions }
