...
export type ToString<
    S,
    Vf extends string = '' /* value to use if conversion (f)ails*/
> = S extends string ? S : S extends number | boolean | bigint ? `${S}` : Vf
...
export type Split<S extends string, D extends string> = string extends S
    ...
    : S extends ''
    ? []
    : S extends `${infer Sx}${D}${infer Sxx}`
    ? [Sx, ...Split<Sxx, D>]
    : [S]
...
export type TrimStart<
    S extends string,
    D extends string = ' '
> = S extends `${D}${infer Sx}` ? Sx : S

export type TrimEnd<
    S extends string,
    D extends string = ' '
> = S extends `${infer Sx}${D}` ? Sx : S

export type Trim<S extends string, D extends string = ' '> = TrimStart<
    TrimEnd<S, D>,
    D
>
