 ...
 */
type ToCamelCase<
    S extends string | string[] | readonly string[],
    D extends string = '-'
> = string extends S
    ? string // ignore untyped strings
    : S extends string
    ? ToCamelCase<Split<S, D>, D>
...
function toCamelCase<S extends string>(
    input: S
): ToCamelCase<S, ' ' | '-' | '_'>

function toCamelCase<S extends string, D extends string>(
    input: S,
    delimeter: D
): ToCamelCase<S, D>
...
function toCamelCase(
    ...
    if (typeof delimiter === 'string') delimiter = new RegExp(delimiter)

    for (let i = 0; i < input.length; i++) {
        const char = input.charAt(i)

        if (delimiter.test(char)) capitalizeNext = true
        else if (capitalizeNext) {
            camelCased += char.toUpperCase()
            capitalizeNext = false
        } else camelCased += char
    }
